...
bool TestMainApp::Initialize()
{
    ...
    ID2D1DeviceContext* d2dContext = m_Renderer->GetD2DContext();
    m_assetManager = std::make_unique<AssetManager>(d2dContext);
    // 타이머 초기화
    m_GameTimer.Reset();
...
void TestMainApp::Render()
{
    ...
    if (m_singleImage)
    {
        D2D1_SIZE_F size = m_singleImage->GetSize();
        ...
        m_Renderer->DrawBitmap(m_singleImage.Get(), dstRect);
    }

    //for (const auto& img : m_staticImages)
    //{
    //    if (img.bitmap)
    //    {
    //        D2D1_SIZE_F size = img.bitmap->GetSize();
    //        D2D1_RECT_F destRect = {
    //            img.position.x,
    //            img.position.y,
    //            img.position.x + size.width,
    //            img.position.y + size.height
    //        };
    //        m_Renderer->DrawBitmap(img.bitmap.Get(), destRect); // renderer에 맞게
    //    }
    //}
    ...
    for (auto& ap : m_curSprites)
    {
        ...
        auto size = ap.GetClip()->GetBitmap()->GetSize();
        std::cout << "[DEBUG] DrawBitmap 호출됨, 텍스처 크기: " << size.width << " x " << size.height << std::endl;
...
void TestMainApp::LoadAssets()
{
    ...
    //확장자 제거한 경로 문자열로 저장 -> 클립 조회/캐싱 키 사용
    ...
    if (keyWide == m_selectedAssetKey) return;

    m_selectedAssetKey = keyPath.wstring(); // 선택된 파일 키 저장

    //애니메이션 로딩
    if (ext == L".json")
    {
        auto clips = m_assetManager->LoadAnimationSet(fullPath);
        if (!clips.empty())
        {
            for (const auto& [name, clip] : clips)
            {
    ...
    {
        m_singleImage = m_assetManager->LoadTexture(fullPath);
        //auto bitmap = m_assetManager->LoadTexture(fullPath);
        //if (bitmap)
        //{
        //    StaticImage img;
        //    img.bitmap = bitmap;
        //    img.position = { 100.0f, 100.0f }; // 기본 위치
        //    m_staticImages.push_back(img);
        //}
    }
